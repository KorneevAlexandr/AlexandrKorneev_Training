...
namespace LibraryBakery
{
	...
	public class Lavash : Bakery
	{
		...
		public Lavash(string[] data_line, double Weight, double Price, double Calorie, int Numerous)
		...
		{
			return String.Format("Лаваш: {0} штук\nМасса: {1} кг\n" +
		...
		public override bool Equals(object obj)
		{
			if (obj == null)
				return false;
			Lavash L = obj as Lavash;
			if (L as Lavash == null)
				return false;

			for (int i = 0; data_line.Length == L.data_line.Length
				&& i < data_line.Length; i++)
			{
				if (data_line[i] != L.data_line[i])
					return false;
			}
			return true;
		}
